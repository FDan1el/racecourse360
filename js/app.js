...
(function probeNightTexture(index) {
    ...
    probe.onload = function () {
        ...
        if (typeof world !== 'undefined' && world && typeof isDayMode !== 'undefined' && !isDayMode) {
...
world = Globe()
    ...
    .polygonSideColor(() => 'rgba(56, 189, 248, 0.35)')
...
function polygonColor(feat, hoverD) {
    ...
    // Adat nélküli országok: láthatatlan, nem kattintható
    if (!DATA_ISO_CODES.includes(iso)) return 'rgba(0, 0, 0, 0)';

    // Rámutatásnál arany kiemelés
    if (feat === hoverD) return 'rgba(245, 158, 11, 0.62)';

    const meta = countryMeta[iso] || {};

    // "Ügető" szűrő: erősebb kék kiemelés
    if (currentFilter === 'trot') {
        return meta.hasTrot ? 'rgba(56, 189, 248, 0.6)' : 'rgba(0, 0, 0, 0)';
    }

    // Alapállapot: JÓL LÁTHATÓ, de visszafogott kék.
    // Korábban 0.10 volt, ami a sötét földgömbön gyakorlatilag
    // láthatatlan lett - ezért nem volt látszólag "színezve", és
    // nem lehetett megtalálni, hová kell kattintani.
...
function polygonStroke(feat) {
    ...
    const meta = countryMeta[iso] || {};
    if (currentFilter === 'trot' && !meta.hasTrot) return 'rgba(0, 0, 0, 0)';

    return '#38bdf8';
}

// ==========================================================
// ORSZÁGHATÁROK (GeoJSON) – TÖBBSZÖRÖS FORRÁSSAL
// ==========================================================
// Ez a projekt legkritikusabb külső adata: enélkül a földgömbön
// egyetlen kattintható ország sincs.
//
// A raw.githubusercontent.com NEM CDN és NEM éles használatra
// szánt: rate-limitelt, és a GitHub bármikor korlátozhatja.
// Ezért a sorrend:
//   1. saját másolat a repóban (a legbiztosabb – lásd README)
//   2. jsDelivr CDN (ugyanaz a fájl, de valódi CDN-ről)
//   3. raw.githubusercontent (utolsó mentsvár)
...
function loadCountries(index) {
    ...
    if (index >= GEOJSON_SOURCES.length) {
        console.error('[RC360] Az országhatárok egyetlen forrásból sem töltődtek be. '
            + 'A földgömb látszik, de országokra kattintani nem lehet. '
            + 'A Pályák menü és a 2D térkép ettől függetlenül működik.');
        return;
    }
    ...
    fetch(url)
        ...
        .then(countries => {
            ...
            // Diagnosztika: ha ez a sor megjelenik a konzolon, a határok
            // betöltődtek. A "felismert" szám mutatja, hány országunkat
            // sikerült azonosítani az ISO-kód alapján (27-nek kell lennie).
            const recognised = countries.features
                .filter(f => DATA_ISO_CODES.includes(resolveIso(f.properties))).length;
            console.log('[RC360] Országhatárok betöltve:', url,
                '| poligonok:', countries.features.length,
                '| ebből felismert (adatolt):', recognised, 'a várt', DATA_ISO_CODES.length, 'közül');
            if (recognised === 0) {
                console.warn('[RC360] FIGYELEM: egyetlen országot sem sikerült azonosítani! '
                    + 'Valószínűleg a GeoJSON mezőnevei mások (ISO_A3 / ADM0_A3).');
            }
        })
        .catch(err => {
            console.log('[RC360] Országhatár-forrás nem elérhető:', url, '-', err.message);
...
function toggleDayNight() {
    isDayMode = !isDayMode;
    if (world) world.globeImageUrl(isDayMode ? TEX_DAY : TEX_NIGHT);
    applyDayNightIcon();
}
