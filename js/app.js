...
// ================================================================
// 6b) RÉGIÓ/ÁLLAM HATÁROK A 2D TÉRKÉPEN
// ================================================================
// CÉL: a földgömbön már látott "van pálya / nincs pálya" színezés és
// kattintás-logika megismétlése ÁLLAM/RÉGIÓ szinten, azoknál az
// országoknál, ahol sok pálya van és van state/régió-bontás.
//
// FORRÁS: Natural Earth 1:50m admin-1 (állam/tartomány/régió) határok.
// UGYANAZ a projekt, UGYANOLYAN 3-lépcsős CDN-lánc, mint az
// országhatároknál (lásd GEOJSON_SOURCES) - ez a legkisebb kockázatú
// választás, mert már bevált mintát követ.
const REGION_ORSZAGOK = ['USA', 'CAN', 'AUS', 'FRA'];

// Csak ennél a 4 országnál kell az ISO3->ISO2 megfeleltetés (nincs
// szükség egy teljes, minden országot lefedő táblára emiatt).
const REGION_ISO3_TO_ISO2 = { USA: 'US', CAN: 'CA', AUS: 'AU', FRA: 'FR' };
...
const REGION_GEOJSON_SOURCES = [
    ...
    'https://cdn.statically.io/gh/nvkelso/natural-earth-vector/master/geojson/ne_50m_admin_1_states_provinces_lakes.geojson',
    'data/regions.geojson'
];

let regionGeoJsonCache = null;   // egyszer töltjük be, utána újrahasználjuk
let regionGeoJsonLoading = false;
let regionLayer = null;          // a jelenleg a térképen lévő Leaflet réteg
let activeRegionFilter = null;   // { iso, key } - ha egy régióra rá van szűrve a lista

// A NE admin-1 fájlban a mezőnevek gyártótól/verziótól függően eltérő
// írásmóddal fordulhatnak elő (KIS/NAGY betű) - ezért mindkettőt próbáljuk.
function ne1Prop(feature, ...nevek) {
...
function featureOrszagIso(feature) {
    return ne1Prop(feature, 'adm0_a3', 'ADM0_A3', 'iso_a2', 'ISO_A2');
}

// A track.region értékével összevethető kulcs kinyerése egy admin-1
// feature-ből. USA/CAN/AUS: a postai/rövid kód (pl. "NY", "VIC", "ON").
// FRA: a hivatalos régiónév (pl. "Normandie") - ott a name mezőt nézzük.
function regionKeyFromFeature(iso, feature) {
    if (iso === 'FRA') {
        return ne1Prop(feature, 'name', 'NAME', 'name_en', 'NAME_EN');
    }
    return ne1Prop(feature, 'postal', 'POSTAL', 'iso_3166_2', 'ISO_3166_2', 'abbrev', 'ABBREV');
}

function loadRegionGeoJson(cb) {
    if (regionGeoJsonCache) { cb(regionGeoJsonCache); return; }
    if (regionGeoJsonLoading) { setTimeout(() => loadRegionGeoJson(cb), 200); return; }
    regionGeoJsonLoading = true;

    const probal = (index) => {
        if (index >= REGION_GEOJSON_SOURCES.length) {
            regionGeoJsonLoading = false;
            console.warn('[RC360] Régióhatárok: egyik forrás sem elérhető - a 2D térkép '
                + 'marad pontjelölős nézetben, a Pályák menü régió-bontása attól még működik.');
            cb(null);
            return;
        }
        const url = REGION_GEOJSON_SOURCES[index];
        fetch(url)
            .then(res => { if (!res.ok) throw new Error('HTTP ' + res.status); return res.json(); })
            .then(data => {
                if (!data || !data.features) throw new Error('hibás GeoJSON szerkezet');
                regionGeoJsonCache = data;
                regionGeoJsonLoading = false;
                console.log('[RC360] Régióhatárok betöltve (' + data.features.length + ' poligon), forrás '
                    + (index + 1) + '/' + REGION_GEOJSON_SOURCES.length, url);
                cb(data);
            })
            .catch(err => {
                console.log('[RC360] Régióhatár-forrás nem elérhető:', url, '-', err.message);
                probal(index + 1);
            });
    };
    probal(0);
...
// Stílus - SZÁNDÉKOSAN ugyanazok a színek, mint a földgömb polygonColor/
// polygonStroke függvényeiben, hogy a két nézet között ne törjön meg az
// élmény. Pálya nélküli régió: teljesen átlátszó ÉS nem interaktív -
// tehát nem is választható, ahogy a földgömbön sem kattintható ország.
function regionStyle(hasData, isHover) {
    if (!hasData) {
        return { fillOpacity: 0, opacity: 0, interactive: false, fillColor: 'transparent', color: 'transparent' };
    }
    const trotAktiv = currentFilter === 'trot';
    if (isHover) {
        return {
            fillColor: trotAktiv ? '#E8BA8C' : '#FFFFFF',
            fillOpacity: trotAktiv ? 0.62 : 0.14,
            color: trotAktiv ? '#7dd3fc' : 'rgba(148,163,184,0.7)',
            weight: 1.5, opacity: 1,
        };
    }
    return {
        fillColor: trotAktiv ? '#D89A5E' : '#FFFFFF',
        fillOpacity: trotAktiv ? 0.5 : 0.16,
        color: trotAktiv ? '#7dd3fc' : 'rgba(148,163,184,0.45)',
        weight: 1, opacity: 1,
    };
}

function removeRegionLayer() {
    if (regionLayer && leafletMap) leafletMap.removeLayer(regionLayer);
    regionLayer = null;
}

function renderRegionLayer(iso) {
    removeRegionLayer();
    if (!REGION_ORSZAGOK.includes(iso)) return;   // csak a 4 nagy országnál

    loadRegionGeoJson(data => {
        // Ha időközben más országra léptünk (vagy visszamentünk a
        // gömbhöz), ne rajzoljunk elavult réteget.
        if (!data || currentIso !== iso || !leafletMap) return;

        const isoLetter2 = REGION_ISO3_TO_ISO2[iso];
        const orszagFeaturek = data.features.filter(f => {
            const azon = featureOrszagIso(f);
            return azon === iso || azon === isoLetter2;
        });

        const regiokAdattal = new Set(trackDatabase[iso].map(t => t.region).filter(Boolean));

        regionLayer = L.geoJSON(orszagFeaturek, {
            style: (feature) => {
                const key = regionKeyFromFeature(iso, feature);
                return regionStyle(!!key && regiokAdattal.has(key), false);
            },
            onEachFeature: (feature, layer) => {
                const key = regionKeyFromFeature(iso, feature);
                const hasData = !!key && regiokAdattal.has(key);
                if (!hasData) return;   // nincs pálya -> se hover, se kattintás

                layer.on('mouseover', () => layer.setStyle(regionStyle(true, true)));
                layer.on('mouseout', () => layer.setStyle(regionStyle(true, false)));
                layer.on('click', () => toggleRegionMapFilter(iso, key, layer));

                const count = trackDatabase[iso].filter(t => t.region === key).length;
                layer.bindTooltip(key + ' — ' + count + ' pálya', { sticky: true });
            },
        }).addTo(leafletMap);

        // A régió-réteg a jelölők ALATT legyen, hogy a pöttyök kattinthatók maradjanak
        if (regionLayer.bringToBack) regionLayer.bringToBack();
    });
}

// Régióra kattintva: rászűrünk a jelölőkre és ráközelítünk. Ugyanarra a
// régióra kattintva mégegyszer: visszaáll a teljes ország-nézet - ez
// felel meg annak, ahogy a földgömbön egy ország kattintása is
// megismételhető/visszavonható élményt ad.
function toggleRegionMapFilter(iso, key, layer) {
    if (activeRegionFilter && activeRegionFilter.iso === iso && activeRegionFilter.key === key) {
        activeRegionFilter = null;
        renderMarkers(iso);
        const b = layer.getBounds();
        if (b.isValid()) leafletMap.fitBounds(regionLayer.getBounds(), { padding: [40, 40], maxZoom: 7 });
        return;
    }
    activeRegionFilter = { iso, key };
    renderMarkers(iso);
    const b = layer.getBounds();
    if (b.isValid()) leafletMap.fitBounds(b, { padding: [50, 50], maxZoom: 9 });
}

// A jelölők (pöttyök) kirajzolása - KÜLÖN függvénybe kiemelve a
// prepareMap-ból, hogy a régió-szűrés újra tudja hívni anélkül, hogy a
// teljes térképet (határréteg, nézet) újra kellene építeni.
function renderMarkers(iso) {
    const meta = countryMeta[iso] || { name: iso };
    const tracks = trackDatabase[iso].filter(t =>
        !activeRegionFilter || activeRegionFilter.iso !== iso || t.region === activeRegionFilter.key);
...
function prepareMap(iso) {
    initLeafletIfNeeded();
    activeRegionFilter = null;   // új ország = nincs aktív régió-szűrés
...
function buildOrszagBlokk(iso) {
    ...
    // ================================================================
    // RÉGIÓ-CSOPORTOSÍTÁS
    // Azoknál az országoknál, ahol sok pálya van és a pályáknak van
    // `region` mezőjük (USA, AUS, CAN, FRA), egy plusz szintet iktatunk
    // be: Ország → Állam/Régió → Pálya. Máshol (ahol nincs region
    // mező) marad a régi, lapos Ország → Pálya lista - ez a régi
    // viselkedés, nem tör el semmit a többi országnál.
    const vanRegio = tracks.some(t => t.region);

    let innerHtml;
    if (vanRegio) {
        // Csoportosítás régió szerint, az EREDETI tömb-indexet megőrizve
        // (jumpToTrack ezt az indexet várja) - ezért map((t,idx)=>...) ELŐBB,
        // csoportosítás UTÁNA.
        const csoportok = new Map();
        tracks.forEach((t, idx) => {
            const kulcs = t.region || '—';
            if (!csoportok.has(kulcs)) csoportok.set(kulcs, []);
            csoportok.get(kulcs).push({ t, idx });
        });
        const regioKulcsok = [...csoportok.keys()].sort((a, b) =>
            a.localeCompare(b, aktualisNyelv || 'hu'));

        innerHtml = regioKulcsok.map(kulcs => {
            const elemek = csoportok.get(kulcs);
            const rows = elemek.map(({ t, idx }) => {
            ...
            return '<button class="tp-region" onclick="toggleRegionGroup(this)">'
                 + '<span>' + kulcs + '</span>'
                 + '<span><span class="tp-count">' + elemek.length + '</span> <span class="tp-arrow">&#9654;</span></span>'
                 + '</button>'
                 + '<div class="tp-region-tracks">' + rows + '</div>';
        }).join('');
    } else {
        innerHtml = tracks.map((t, idx) => {
            const st = STATUS_TEXT[t.status || 'active'] || STATUS_TEXT.active;
            return '<button class="tp-track" onclick="jumpToTrack(\'' + iso + '\',' + idx + ')">'
                 + '<span class="tp-tname">' + t.name + '</span>'
                 + '<span class="tp-tmeta">' + t.city + '</span>'
    ...
    return '<button class="tp-country" onclick="toggleCountryGroup(this)">'
         ...
         + '</button>'
         + '<div class="tp-tracks">' + innerHtml + '</div>';
}
...
function toggleRegionGroup(btnEl) {
    const list = btnEl.nextElementSibling;
    const isOpen = list.classList.contains('open');
    const sajatOrszagDoboz = btnEl.closest('.tp-tracks');
    if (sajatOrszagDoboz) {
        sajatOrszagDoboz.querySelectorAll('.tp-region-tracks').forEach(el => el.classList.remove('open'));
        sajatOrszagDoboz.querySelectorAll('.tp-region').forEach(el => el.classList.remove('open'));
    }
    if (!isOpen) { list.classList.add('open'); btnEl.classList.add('open'); }
}
