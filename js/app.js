...
function polygonColor(feat, hoverD) {
    ...
    if (hasData && feat === hoverD) return 'rgba(245, 158, 11, 0.55)';
...
const GEOJSON_SOURCES = [
    // 1) Saját másolat a repóban – a legbiztosabb, nulla külső függőség.
    //    (Ha még nincs feltöltve, ez 404-et ad, és a lánc továbblép.)
    'data/countries.geojson',
    // 2-4) UGYANANNAK a Natural Earth fájlnak három különböző tükre.
    //    Fontos, hogy mindhárom ugyanaz a fájl legyen: a kódunk az
    //    ISO_A3 / ADM0_A3 mezőkre épül, egy más szerkezetű GeoJSON
    //    elrontaná az országfelismerést.
...
function loadCountries(index) {
    ...
    if (index >= GEOJSON_SOURCES.length) {
        const msg = 'HIBA: az országhatárok egyetlen forrásból sem töltődtek be. '
                  + 'A Pályák menü és a 2D térkép ettől függetlenül működik.';
        console.error('[RC360] ' + msg);
        setGeoStatus(msg, 'fail');
        return;
    }

    const url = GEOJSON_SOURCES[index];
    const shortUrl = url.length > 46 ? '…' + url.slice(-44) : url;
    setGeoStatus('Próba ' + (index + 1) + '/' + GEOJSON_SOURCES.length + ': ' + shortUrl);
    ...
    fetch(url)
        ...
        .then(countries => {
            ...
            if (!world) throw new Error('a földgömb nem jött létre (world = undefined)');
            ...
            const msg = 'Betöltve: ' + countries.features.length + ' poligon | '
                      + 'felismert: ' + recognised + '/' + DATA_ISO_CODES.length
                      + ' | forrás ' + (index + 1);
            console.log('[RC360] ' + msg, url);

            if (recognised === 0) {
                setGeoStatus(msg + ' — FIGYELEM: egyetlen országot sem sikerült '
                    + 'azonosítani, a GeoJSON mezőnevei mások lehetnek.', 'warn');
            } else {
                setGeoStatus(msg, 'ok');
                // Siker esetén 12 másodperc után elhalványítjuk, hogy ne zavarjon
                setTimeout(() => {
                    const el = document.getElementById('geoStatus');
        ...
        .catch(err => {
            ...
            setGeoStatus('Próba ' + (index + 1) + ' sikertelen (' + err.message
                + '), következő forrás…', 'warn');
