 ...
 */
function mutasdOrszagInfot(iso) {
    const meta = countryMeta[iso] || { name: iso };
    const db = trackDatabase[iso] || [];
    const ugeto = db.filter(t => t.status !== 'closed').length;
    const sav = document.getElementById('instructionBar');
    if (!sav) return;

    sav.innerHTML =
        `<b>${meta.name}</b>: ${ugeto} dokumentált <b>ügető</b> versenypálya`
        + `<span class="ib-hint">A megtekintéshez kapcsold be fent a `
        + `<b>Ügető</b> szűrőt (a fejléc bal oldalán)</span>`;
    sav.classList.add('ib-info');
    sav.style.opacity = '1';
}
...
function setLanguage(code, btn, isManual = true) {
    ...
    // Az utasítássáv HTML-t tartalmaz (kiemelés + magyarázó sor), ezért
    // NEM setText-tel írjuk felül – az elrontaná a formázást.
    const ib = document.querySelector('#instructionBar');
    if (ib && t.instruction && !ib.classList.contains('ib-info')) {
        ib.innerHTML = t.instruction;
    }
