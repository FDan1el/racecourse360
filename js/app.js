...
function polygonColor(feat, hoverD) {
    ...
    // Adat nélküli országok: gyakorlatilag láthatatlanok, nem kattinthatók
    if (!DATA_ISO_CODES.includes(iso)) return 'rgba(0, 0, 0, 0)';

    // Rámutatásnál arany kiemelés
    if (feat === hoverD) return 'rgba(245, 158, 11, 0.55)';

    const meta = countryMeta[iso] || {};

    // "Ügető" szűrő: kék kiemelés (erre van megbízható adatunk)
    if (currentFilter === 'trot') {
        return meta.hasTrot ? 'rgba(56, 189, 248, 0.55)' : 'rgba(0, 0, 0, 0)';
    }

    // Alapállapot: NAGYON halvány kitöltés - a földgömb textúrája átüt,
    // tehát nincs "beszínezve", de az ország azért érzékelhető, hogy a
    // látogató tudja, hová kattinthat. A tényleges jelzést a világító
    // határvonal adja (lásd polygonStroke).
