...
function kiemelkedesAnimacio(feat, utana) {
    ...
    world.polygonsTransitionDuration(260);
    world.polygonAltitude(d => (d === feat ? POLY_ALT_CLICK : POLY_ALT_BASE));

    setTimeout(() => {
        world.polygonsTransitionDuration(420);
        world.polygonAltitude(d => {
            const iso = resolveIso(d.properties);
            return DATA_ISO_CODES.includes(iso) ? POLY_ALT_BASE : POLY_ALT_BASE;
        });
        setTimeout(() => world.polygonsTransitionDuration(POLY_TRANS_ALAP), 420);
    }, 300);

    if (utana) setTimeout(utana, 180);   // a művelet a csúcs előtt indul
}
...
world = Globe()
    ...
    .polygonLabel(({ properties }) => {
        ...
        // Ha még nincs kiválasztva szakág, a címke maga mondja meg a teendőt.
        // Így az utasítás ott jelenik meg, ahová a látogató épp néz –
        // nem kell külön sávot olvasnia a képernyő alján.
        const utasitas = currentFilter
            ? ''
            : `<div style="margin-top:7px;padding-top:7px;border-top:1px solid rgba(216,154,94,0.3);
                 color:#c9b8a8;font-size:0.76rem;line-height:1.45;">
                 A megtekintéshez válassz szakágat:<br>
                 kattints fent az <b style="color:#D89A5E;">Ügető</b> gombra
               </div>`;
        return `
            <div style="background:rgba(62,35,24,0.97);color:#f0e6dd;padding:10px 14px;
                ...
                <strong style="font-size:0.95rem;">${meta ? meta.name : iso}</strong><br/>
    ...
    .onPolygonClick(polygon => {
        ...
        if (!currentFilter) {
            // Szakág nélkül nem megyünk 2D térképre – de a kiemelkedés
            // visszajelzi, hogy a kattintás célba ért.
