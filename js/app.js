 ...
 */
async function idojarastBetolt(elemAzonosito, lat, lng) {
    let adat;
    try {
    ...
    const el = document.getElementById(elemAzonosito);
    if (!el) return;   // a popup közben bezárult
...
function buildPopupHtml(t, meta, orszagKod) {
    ...
    const idojarasAzonosito = 'pw-' + orszagKod + '-' + (t.slug || Math.random().toString(36).slice(2));
    const idoSor = idoSzoveg
        ? `<div class="popup-idosor">
                <span class="popup-ido-blokk">
                    ...
                    (zonaNev ? `<span class="popup-zona">${zonaNev}</span>` : '') +
                `</span>
                <span class="popup-idojaras" id="${idojarasAzonosito}"></span>
            </div>`
        : '';

    /* A lekérés a popup megjelenése UTÁN indul: a HTML még csak
       most készül el, az elem a DOM-ban még nem létezik. */
    if (idoSzoveg && t.lat != null && t.lng != null) {
        setTimeout(() => idojarastBetolt(idojarasAzonosito, t.lat, t.lng), 0);
    }
