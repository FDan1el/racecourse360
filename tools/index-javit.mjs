...
/* ------------------------------------------------------------
   1. HAMBURGER MENÜ
   ------------------------------------------------------------ */
const MENU_REGI = `<div class="dropdown-menu" id="hamburgerMenu">
...
function main() {
    ...
    /* --- 1. Menü --- */
    if (s.includes(MENU_UJ)) {
        kesz.push('A hamburger menü MÁR javítva van – kihagyva.');
    } else if (s.includes(MENU_REGI)) {
        s = s.replace(MENU_REGI, MENU_UJ);
        kesz.push('Hamburger menü: gombok → <a href> linkek (+3 új menüpont)');
    } else {
        hibak.push(
            'A hamburger menü blokkja nem található a várt formában.\n' +
            '      Lehet, hogy időközben kézzel módosult. Ellenőrizd kézzel.'
        );
    }
