...
/* A hirdetési sáv váza. A #reklamTartalom-ba kerül majd a
   tényleges hirdetéskód (AdSense vagy affiliate blokk). */
const REKLAM_SAV = `<div id="reklamSav" aria-hidden="true">
<span class="reklam-cimke" data-i18n="hirdetes">Hirdetés</span>
<div class="reklam-keret" id="reklamTartalom"></div>
</div>
`;
...
function main() {
    ...
    /* --- 6. HIRDETÉSI SÁV HELYE ---
       A földgömb alatti rögzített sáv szerkezete. ALAPBÓL REJTETT
       (a CSS display:none-nal indul), és csak akkor jelenik meg,
       ha az app.js reklamSavAktival() függvényét meghívják.

       Most üresen kerül be: forgalom nélkül a hirdetés csak ront
       a benyomáson, és az AdSense sem fogad be üres oldalt. A
       helynek viszont készen kell állnia, hogy élesítéskor ne
       kelljen az elrendezéshez nyúlni. */
    if (s.includes('id="reklamSav"')) {
        kesz.push('A hirdetési sáv MÁR benne van – kihagyva.');
    } else {
        const bodyVeg = s.lastIndexOf('</body>');
        if (bodyVeg === -1) {
            hibak.push('Nem található </body> – nincs hova tenni a hirdetési sávot.');
        } else {
            s = s.slice(0, bodyVeg) + REKLAM_SAV + s.slice(bodyVeg);
            kesz.push('Hirdetési sáv helye létrehozva (rejtett, élesítésre vár)');
        }
    }
