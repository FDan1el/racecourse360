...
const FORRASOK = [
  ...
  },
  {
    ...
    ad: ['koordinata', 'alapitas'],
    orszagok: () => true,
    megbizhatosag: 'magas',
    sorrend: 6,
    lekerdez: (palya, ctx) => koordWikipedia(palya, ctx.nyelvek),
  },
  {
    nev: 'GeoNames',
    csalad: 'geonames',
  ...
  },
  {
    ...
    ad: ['hossz', 'irany', 'talaj', 'celegyenes', 'szelesseg', 'honlap'],
 ...
 */
async function kiegeszitoMod(db, orszagok) {
  ...
  for (const orszag of orszagok) {
    for (const p of (db[orszag] || [])) {
      ...
      if (hianyzo.length) feladat.push({ orszag, palya: p, hianyzo });
    }
  }
  ...
  for (const f of feladat) {
    n++;
    process.stdout.write(`[${String(n).padStart(3)}/${feladat.length}] ${f.orszag} – ${f.palya.name} ... `);
    ...
    if (Object.keys(talalt).length) {
      eredmenyek.push({ orszag: f.orszag, palya: f.palya.name, talalt });
    }
  }

  await fs.mkdir(CONFIG.outDir, { recursive: true });
  const csvSor = (m) => m.map((x) => `"${String(x ?? '').replace(/"/g, '""')}"`).join(';');
  const sorok = [csvSor(['Ország', 'Pálya', 'Mező', 'Javasolt érték', 'Ütközés', 'Források'])];
  for (const e of eredmenyek) {
    for (const [mezo, adat] of Object.entries(e.talalt)) {
      sorok.push(csvSor([e.orszag, e.palya, mezo, adat.ertek ?? '',
