...
const HOST_KORLAT = {
  ...
  'overpass-api.de': 1500,   // közösségi szolgáltatás – legyünk kíméletesek
...
out center 300;`;
  try {
    ...
    console.log(`    (Overpass hiba: ${e.message})`);
    return [];
  }
}
...
async function koordinataGyujtes(palya, orszagKod, nyelvek) {
  ...
  // Overpass: a ténylegesen felrajzolt versenypálya-objektum.
  // Településre soha nem téved – ezért erős forrás.
  const ovp = await koordOverpass(palya, orszagKod);
  if (ovp) talalatok.push(ovp);
  await varj(CONFIG.delayMs);
...
async function wikidataFelfedezes(orszagKod) {
  ...
  const szuro = UGETO_KULCSSZAVAK
    .map((k) => `CONTAINS(LCASE(?szoveg), "${k}")`)
    .join(' || ');

  // Az országot ANGOL CÍMKÉVEL kötjük, nem Q-számmal – így nem kell
  // azonosítókat fejből tudni, és nem lehet elgépelni őket.
  const query = `
    SELECT DISTINCT ?item ?itemLabel ?coord ?leiras ?honlap ?alapitas WHERE {
      ...
      ?item rdfs:label ?cimke . FILTER(LANG(?cimke) = "en")
      BIND(CONCAT(?cimke, " ", COALESCE(?leiras, "")) AS ?szoveg)
      FILTER(${szuro})
...
async function felfedezoMod(db, orszagok) {
  ...
  for (const orszag of orszagok) {
    ...
    console.log(`${talalatok.length} találat (Wikidata + OSM), ebből ${ujak.length} nálunk MÉG NINCS`);
    for (const t of ujak) {
      mind.push({ orszag, ...t });
    }
    await varj(CONFIG.delayMs);
  ...
  const sorok = [csvSor(['Ország', 'Forrás', 'Azonosító', 'Név', 'Leírás', 'lat', 'lng', 'Alapítás', 'Honlap'])];
  for (const t of mind) {
    sorok.push(csvSor([t.orszag, t.honnan || 'Wikidata', t.qid, t.name, t.leiras, t.lat, t.lng, t.founded, t.ownSite]));
  }
  await fs.writeFile(path.join(CONFIG.outDir, 'felfedezes.csv'), '\uFEFF' + sorok.join('\r\n'), 'utf8');

  // Közvetlenül a jelöltlistába illeszthető formátum
  const jeloltek = mind.map((t) => ({
  ...
  console.log(`\n=== ${mind.length} olyan pálya, ami nálunk még nincs meg ===`);
  for (const t of mind) {
