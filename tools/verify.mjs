...
function koordOsszevet(talalatok, sajatLat, sajatLng) {
  ...
  if (fo.length >= 2) {
    // A legmegbízhatóbb forrást választjuk az egyetértő csoportból
    ...
    if (sajat) {
      ...
      if (d > KOORD_EGYEZES_KM) {
        return { allapot: 'SAJÁT ADAT GYANÚS', javasolt: legjobb, tavolsagKm: d,
                 reszletek: [sajat, ...jo],
                 teendo: `${fo.length} független forrás egyetért egymással, de a mi koordinátánk ${d.toFixed(1)} km-re van tőlük. Valószínűleg a MI adatunk hibás.${kilogoSzoveg}` };
      }
    ...
    return { allapot: 'JAVASOLHATÓ', javasolt: legjobb, reszletek: jo,
             teendo: `${fo.length} független forrás egyetért.${kilogoSzoveg} Felvehető, de a pálya azonosságát érdemes szemre is ellenőrizni.` };
  }
