...
out center 8;`;
  ...
  try {
    ...
    const c = legjobb.cimkek;

    // ÚJ: ha az OSM-objektumon vannak műszaki cimkék, azokat is kiolvassuk.
    // Ez NEM plusz hálózati kérés – ugyanabból a válaszból van, amit már
    // úgyis a koordinátáért letöltöttünk. A lefedettség esetleges (attól
    // függ, mennyire alapos volt a térképész), de ahol van, ingyen van.
    const kiegeszito = {};
    // hossz méterben: "length"=1000 vagy "distance"=1000, esetleg "1000 m"
    const hosszCimke = c.length ?? c.distance;
    if (hosszCimke) {
      const szam = parseFloat(String(hosszCimke).replace(/[^\d.]/g, ''));
      if (szam >= 300 && szam <= 3000) kiegeszito.length = Math.round(szam);
    }
    // talaj: OSM-ben jellemzően angolul (sand, dirt, grass, clay, gravel)
    const TALAJ_FORDITAS = {
      sand: 'homok', dirt: 'salak', clay: 'salak', grass: 'fű',
      gravel: 'zúzalék', ash: 'salak', turf: 'fű',
    };
    if (c.surface && TALAJ_FORDITAS[c.surface.toLowerCase()]) {
      kiegeszito.surface = TALAJ_FORDITAS[c.surface.toLowerCase()];
    }
    // irány: "oneway"="yes"/"-1" a rajzolt vonal irányát adja, ami önmagában
    // nem elég (a vonal rajzolási iránya nem feltétlenül a versenyirány) –
    // ezért ezt SZÁNDÉKOSAN nem alakítjuk "left"/"right"-tá, csak megjegyzésbe
    // kerül, emberi ellenőrzésre.
    ...
    return {
      ...
      megbizhatosag: 'magas',
      megjegyzes: `OSM-címkék: ${Object.entries(c)
        .filter(([k]) => ['name', 'leisure', 'sport', 'landuse', 'length', 'distance', 'surface', 'oneway'].includes(k))
        .map(([k, v]) => `${k}=${v}`).join(', ')}`,
    };
  } catch {
    return null;
