...
// Franciaország befoglaló doboza (Korzikával együtt).
const FR_BBOX = { del: 41.0, nyugat: -5.6, eszak: 51.6, kelet: 9.8 };
...
async function osmPalyakLetoltese() {
  ...
  const { del, nyugat, eszak, kelet } = FR_BBOX;
  const bbox = `${del},${nyugat},${eszak},${kelet}`;
  const query = `
    [out:json][timeout:900];
    (
      nwr(${bbox})["leisure"="track"]["sport"~"equestrian|horse_racing"];
      nwr(${bbox})["landuse"="racetrack"];
      nwr(${bbox})["sport"="horse_racing"];
      nwr(${bbox})["name"~"[Hh]ippodrome"];
    );
    out center tags;
  `.trim();
