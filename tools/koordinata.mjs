...
// ABSZOLÚT HATÁR: a talált pálya SOHA nem lehet ennél messzebb attól a
// településtől, amit az Excel megad. Enélkül a névegyezés országos szinten
// fut, és 500 km-re lévő, azonos nevű pályát is elfogad (SAINT-OMER,
// VITRE, CAEN esetei a korábbi futásban).
const TELEPULES_MAX_TAVOLSAG_M = 25000;
...
async function telepulesKoordinata(telepules) {
  ...
  try {
    const url =
      `https://nominatim.openstreetmap.org/search?` +
      `q=${encodeURIComponent(`${telepules}, ${ORSZAG_NEV}`)}` +
      `&format=json&limit=1&countrycodes=${ORSZAG_KOD}`;
    const resp = await fetch(url, { headers: { 'User-Agent': USER_AGENT } });
    if (resp.ok) {
      const json = await resp.json();
      if (json?.[0]) {
        eredmeny = { lat: parseFloat(json[0].lat), lon: parseFloat(json[0].lon) };
      }
    }
  } catch {
    // marad null
...
function legjobbOsmJelolt(osmLista, nev, horgonyLat, horgonyLon, telepulesKoord) {
  let legjobb = null;
  let legjobbPont = -Infinity;
  const jeloltek = [];

  for (const o of osmLista) {
    // 1) KEMÉNY SZŰRŐ: a településtől mért távolság. Ezen nem lehet
    //    átlépni névegyezéssel sem — enélkül 500 km-re lévő, azonos nevű
    //    pálya is bekerülne.
    if (telepulesKoord) {
      const tavTelepules = tavolsagMeter(
        telepulesKoord.lat, telepulesKoord.lon, o.lat, o.lon
      );
      if (tavTelepules > TELEPULES_MAX_TAVOLSAG_M) continue;
    }

    // 2) A horgony (a névkeresés találata, ha van) a finomhangoláshoz kell.
    const tav =
      horgonyLat != null
    ...
    // Ha nincs horgony, a településszűrő már elvégezte a helyi szűkítést,
    // ezért itt elég a névegyezés vagy a puszta közelség.
    if (!kozel && h < 0.3 && horgonyLat != null) continue;
...
async function main() {
  ...
  for (const sor of sorok) {
    ...
    try {
      ...
      // A2) A település koordinátája — ez a KÖTELEZŐ józanész-horgony.
      const telepulesKoord = await telepulesKoordinata(telepules);

      // A3) Ha a névkeresés a településtől messze talált valamit, azt is
      //     eldobjuk — a Nominatim is tévedhet (pl. azonos nevű pálya
      //     az ország másik végén).
      if (nevTalalat && telepulesKoord) {
        const tavT = tavolsagMeter(
          telepulesKoord.lat, telepulesKoord.lon, nevTalalat.lat, nevTalalat.lon
        );
        if (tavT > TELEPULES_MAX_TAVOLSAG_M) {
          e.koord_megjegyzes =
            `A névkeresés találata ${Math.round(tavT / 1000)} km-re volt ` +
            `${telepules} településtől — eldobtuk. `;
          nevTalalat = null;
        }
      }
      ...
      const elozoMegjegyzes = e.koord_megjegyzes || '';
      ...
      if (nevTalalat && legjobb && legjobb.tav_m != null && legjobb.tav_m <= EGYEZES_TAVOLSAG_M) {
        ...
        e.koord_lat = legjobb.lat;
        e.koord_lon = legjobb.lon;
        e.koord_forras = 'OSM pálya-objektum (névegyezés, településhez közel)';
        e.koord_megjegyzes =
          elozoMegjegyzes +
          `OSM pálya-objektum erős névegyezéssel: "${legjobb.nev}". ` +
          `A településtől számított 25 km-es határon belül van.`;
      } else if (nevTalalat) {
        ...
        e.koord_megjegyzes =
          elozoMegjegyzes +
          `Van pálya-objektum a település közelében ("${legjobb.nev || 'névtelen'}"), ` +
          `de a név nem egyezik erősen. Ellenőrizd.`;
      } else {
        e.koord_allapot = 'NINCS_TALALAT';
        e.koord_megjegyzes =
          elozoMegjegyzes +
          (telepulesKoord
            ? 'A településtől 25 km-en belül nincs OSM pálya-objektum, és a névkeresés sem talált.'
