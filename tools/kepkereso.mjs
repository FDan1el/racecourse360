...
<style>
 body{background:#1a0f0a;color:#f0e6dd;font:15px/1.5 system-ui,sans-serif;margin:0;padding:16px 16px 150px}
 ...
 .kepek{display:flex;gap:12px;overflow-x:auto;padding-bottom:6px;-webkit-overflow-scrolling:touch}
 figure{margin:0;flex:0 0 240px;background:#2a1810;border-radius:6px;overflow:hidden;
        border:3px solid transparent;transition:border-color .15s}
 figure.kijelolve{border-color:#22c55e}
 figure label{position:relative;display:block;cursor:pointer}
 figure img{width:100%;height:150px;object-fit:cover;display:block}
 /* A pipa NAGY es mindig latszik - a natv checkbox mobilon tul apro */
 .pipa{position:absolute;top:8px;left:8px;width:34px;height:34px;border-radius:50%;
       background:rgba(0,0,0,.6);border:2px solid #fff;display:flex;align-items:center;
       justify-content:center;font-size:20px;color:transparent;pointer-events:none}
 figure.kijelolve .pipa{background:#22c55e;border-color:#fff;color:#fff}
 figure input{position:absolute;opacity:0;width:100%;height:100%;top:0;left:0;margin:0;cursor:pointer}
 ...
 #sav{position:fixed;left:0;right:0;bottom:0;background:#3E2318;border-top:1px solid #D89A5E;
      padding:10px 14px;display:flex;gap:10px;align-items:center;flex-wrap:wrap;z-index:10}
 button{background:#D89A5E;color:#1a0f0a;border:none;padding:11px 16px;border-radius:3px;
        font-weight:700;cursor:pointer;font-size:0.9rem}
 button.mas{background:#5a3a28;color:#f0e6dd}
 #db{color:#c9b8a8;font-size:0.85rem}
 #ki{display:none;width:100%;height:130px;margin-top:8px;background:#1a0f0a;color:#f0e6dd;
     border:1px solid #D89A5E;border-radius:3px;padding:8px;font:12px monospace}
 #ki.latszik{display:block}
</style></head><body>
...
${kartyak}
<div id="sav">
 <button onclick="masol()">Másolás</button>
 <button class="mas" onclick="letolt()">Letöltés</button>
 <button class="mas" onclick="mutat()">Mutasd</button>
 <button class="mas" onclick="torol()">Törlés</button>
 <span id="db">0 kép kijelölve</span>
 <textarea id="ki" readonly></textarea>
</div>
<script>
const adat = ${JSON.stringify(eredmenyek)};
const KULCS = 'rc360_kepvalogato_${iso}';

/* A kijelolest elmentjuk a bongeszobe: tobb szaz palya atnezese kozben
   egy veletlen frissites vagy appvaltas ne torolje el a munkat. */
function betolt(){
  try{
    JSON.parse(localStorage.getItem(KULCS) || '[]').forEach(k => {
      const cb = document.querySelector('input[data-palya="'+k[0]+'"][data-kep="'+k[1]+'"]');
      if(cb){ cb.checked = true; jelol(cb); }
    });
  }catch(e){}
}
function ment(){
  const m = [];
  document.querySelectorAll('input:checked').forEach(cb => m.push([cb.dataset.palya, cb.dataset.kep]));
  try{ localStorage.setItem(KULCS, JSON.stringify(m)); }catch(e){}
}
function jelol(cb){ cb.closest('figure').classList.toggle('kijelolve', cb.checked); }
function frissit(){
  document.getElementById('db').textContent =
    document.querySelectorAll('input:checked').length + ' kép kijelölve';
}
document.addEventListener('change', e => {
  if(e.target.type === 'checkbox'){ jelol(e.target); ment(); frissit(); }
});

function gyujt(){
  const ki = [];
  document.querySelectorAll('input:checked').forEach(cb => {
    const p = adat[cb.dataset.palya], t = p.talalatok[cb.dataset.kep];
    ki.push({ nev:p.nev, varos:p.varos, fajlOldal:t.fajlOldal, licenc:t.licenc, szerzo:t.szerzo });
  });
  return ki;
}

/* MASOLAS
   Az execCommand('copy') kepernyon kivuli textarea-val iOS Safariban NEM
   mukodik. Itt a modern Clipboard API-t hasznaljuk, es ha az sem elerheto
   (nem HTTPS, regi bongeszo), a szoveget LATHATOVA tesszuk. */
async function masol(){
  const ki = gyujt();
  if(!ki.length){ alert('Még nincs kijelölt kép.'); return; }
  const szoveg = JSON.stringify(ki, null, 1);
  const ta = document.getElementById('ki');
  ta.value = szoveg;
  try{
    await navigator.clipboard.writeText(szoveg);
    alert(ki.length + ' kép kimásolva a vágólapra.');
  }catch(e){
    ta.classList.add('latszik');
    ta.focus(); ta.setSelectionRange(0, szoveg.length);
    alert('A vágólap nem érhető el. A szöveg megjelent alul - jelöld ki és másold kézzel.');
  }
}
/* LETOLTES - a legbiztosabb ut mobilon, ha a vagolap akadozik. */
function letolt(){
  const ki = gyujt();
  if(!ki.length){ alert('Még nincs kijelölt kép.'); return; }
  const a = document.createElement('a');
  a.href = URL.createObjectURL(new Blob([JSON.stringify(ki, null, 1)], {type:'application/json'}));
  a.download = 'valasztott_kepek.json';
  document.body.appendChild(a); a.click(); a.remove();
}
function mutat(){
  const ta = document.getElementById('ki');
  ta.value = JSON.stringify(gyujt(), null, 1);
  ta.classList.toggle('latszik');
}
function torol(){
  if(!confirm('Biztosan törlöd az összes kijelölést?')) return;
  document.querySelectorAll('input:checked').forEach(cb => { cb.checked=false; jelol(cb); });
  ment(); frissit();
}
betolt(); frissit();
</script>
