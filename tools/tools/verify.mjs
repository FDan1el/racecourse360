...
async function beolvasAdatbazis() {
  const kod = await fs.readFile(CONFIG.appJsPath, 'utf8');
...
async function beolvasUjPalyak(fajl) {
  try {
    const txt = await fs.readFile(fajl, 'utf8');
    const lista = JSON.parse(txt);
    if (!Array.isArray(lista)) throw new Error('a fájl nem tömböt tartalmaz');
    return lista;
  } catch (e) {
    throw new Error(`Nem tudom beolvasni a jelöltlistát (${fajl}): ${e.message}`);
  }
}
